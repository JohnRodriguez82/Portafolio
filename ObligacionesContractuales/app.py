...
def config():
    ...
    IMPORTANTE:
    - GET /config NO realiza llamadas a Gemini.
    - La API Key solo se valida cuando el usuario la guarda.
    - Esto evita que cada entrada a Configuración dependa de Internet/Gemini.
    """
    ...
    # Esta operación es local y no consulta Gemini.
    api_key = _obtener_api_key()

    # En GET solamente informamos si existe una API Key.
    # NO hacemos verificar_api_key() aquí.
    api_key_valida = False
    api_key_error = ''
    modelo_disponible = ''

    if api_key:
        # La clave está configurada, pero no se verifica contra Gemini
        # al abrir la página.
        api_key_valida = True
    ...
    if request.method == 'POST':
        ...
        if action == 'agregar_obligacion':
            if not contrato:
                flash(
                    'Primero debe configurar el contrato.',
                    'danger'
                )
                return redirect(url_for('contratos'))
            ...
            # Validar que el número de obligación no se repita
            # dentro del contrato actual.
            existente = Obligacion.query.filter_by(
                numero=numero,
        ...
        # ============================================================
        # GUARDAR / ACTUALIZAR API KEY
        # ============================================================
        elif action == 'guardar_api_key':
            api_key_raw = request.form.get(
                'gemini_api_key',
                ''
            )

            api_key_nueva = _limpiar_key_externa(
                api_key_raw
            )

            # --------------------------------------------------------
            # Si el usuario está eliminando la API Key
            # --------------------------------------------------------
            if not api_key_nueva:
                _guardar_api_key('')
            ...
            # --------------------------------------------------------
            # IMPORTANTE:
            # Aquí SÍ hacemos la llamada a Gemini.
            #
            # El usuario acaba de guardar/actualizar la clave,
            # por lo que es el momento correcto para validarla.
            # --------------------------------------------------------
            try:
                valida, mensaje = verificar_api_key(
                    api_key_nueva
                )
            except Exception as e:
                valida = False
                mensaje = (
                    'No fue posible verificar la API Key de Gemini. '
                    f'Detalle: {str(e)}'
                )

            if valida:
                _guardar_api_key(api_key_nueva)

                flash(
                    'API key de Gemini configurada y '
            ...
            flash(
                f'Error al verificar la API key: {mensaje}',
                'danger'
            )

            # No guardamos una clave que no pudo ser validada.
            return redirect(url_for('config'))
    ...
    #
    # MUY IMPORTANTE:
    # No llamar verificar_api_key() aquí.
    # ================================================================
